...
pub(crate) fn wrap_rustc_helper() -> Result<i32> {
    ...
    // Filter out "Compiling" messages from stderr since cargo already displays these
    // via tee_error_line. Showing them again here causes duplicate output.
...
///
/// Previously this would show "Compiling xxx" messages, but for a cleaner
/// REPL experience, we now suppress all such output.
fn tee_error_line(_line: &[u8]) {
    // Suppress all "Compiling" messages for cleaner REPL output.
    // The original behavior was to show dependency crate compilations,
    // but this creates noise in the REPL. Errors are still displayed
    // via errors_from_cargo_output.
}
